...
The app currently ships as **Drums Only Metronome**. The business goal is to sell branded versions to local drum shops so each shop can offer its customers a metronome app under its own identity.

This is not intended to become a multi-purpose shop platform. The product is the metronome. Branding, packaging, deployment, and maintenance are what vary for each shop.

## Commercial model

- **Customer:** a local drum shop
- **End user:** the shop's drummer customers
- **Offer:** the shop's own branded version of the metronome
- **Shared product:** one maintained metronome, training, and audio codebase
- **Shop-specific layer:** name, logo, colors, typography, icons, installable-app metadata, web address, and deployment

Drums Only is the reference implementation. The remaining work is to turn the current reskin boundary into a repeatable shop-build and deployment process.
...
- Semantic canvas, surface, text, accent, muted, border, focus, and secondary-rhythm colors
- Heading and body font stacks

`src/components/layout/PhoneFrame.jsx` applies that definition at the app-shell boundary. It resolves public assets against Vite's deployment base and exposes semantic `--brand-*` CSS properties. Pulse styling across Metronome, Training, Settings, transport, and navigation consumes those semantic properties; Tailwind aliases remain available for legacy utility styles.
