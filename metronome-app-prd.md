...
Wheels use direct horizontal touch/mouse dragging at one consistent 24 px-per-step scale and coast naturally after release. Grabbing during momentum stops the cylinder at its current position and allows immediate reversal. The selected integer is published while dragging and coasting; acknowledging that setting must not reset the animation. Release, blur and pointer-capture loss retain the current selection. The wheel settles to the nearest integer without a CSS replay or rollback to the initial value. Only explicit external settings changes reset it. There are no special flick timing rules, detents, direction locks, per-range scales or trackpad-tail suppression. Mouse/trackpad input uses native momentum and settles after 140 ms of inactivity. Reduced motion skips release animations. Arrow keys and inset minus/plus buttons change one step, Page Up/Down ten, and Home/End select bounds; all stop momentum. Tapping the center or pressing Enter opens numeric entry: Enter/blur saves a whole number clamped to bounds, Escape cancels the edit, and invalid input preserves the old value. Outside text entry, Escape stops motion at the current number. A small tap tolerance does not delay direct movement. Touch page scrolling remains available vertically. Grip texture and numbers share the same continuous position; a square drum-key-style marker indicates the selected value. Frame-batched updates follow existing audio-engine bar-boundary rules. No animation runs while idle.

All three cards use the selected side-view throw-off switch, mounted flush to the right edge and anchored at the folded header while additional settings expand below it. Its stationary 64 × 114 px button controls trainer state immediately. The hardware slides down 3 px and swings outward when off, then rises and closes when on (270 ms release / 210 ms engagement). Accessible names and `aria-checked` replace visible On/Off wording. Keyboard activation, disabling and reduced motion are retained. Narrow cards place header wheels across the full available width; regular wheels are 44.2 px tall and compact wheels are 37.4 px tall (15% shorter); other controls retain their existing touch heights.
