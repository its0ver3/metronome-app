...
- Subdivisions use the approved notation across the rhythm sheet, rhythm pill, trainer preview, and stages. One notation group spans a written quarter or eighth note, according to the meter denominator, with 1–13 clicks. Compact 76 × 44 px dropdowns use the Blue Olive accent and accessible musical-name/count labels. /8 meters also offer group-pulse-only playback. Bar counts remain numeric wheels. There is no More section, custom-meter editor, or tempo-unit selector; BPM is fixed to quarter notes. Uneven-group choices appear only for 5/8 and 7/8. Static SVG noteheads derive from licensed Bravura outlines, without a full runtime font or extra animation.
