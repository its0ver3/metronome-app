...
- Standard mode divides the orbit into meter groups proportional to their written duration, with parallel gaps. Simple beats remain equal; a 7/8 grouping of 2+2+3 has two shorter segments and one longer segment. The same geometry is used in the mini transport.
...
- The Rhythm sheet opens with the selected Soft Lift motion: a calm 440 ms compositor-driven rise and 300 ms opacity-only backdrop fade. Dismissal returns the sheet and fades the backdrop in 220 ms, with no bounce, so the main controls become available sooner. The backdrop intentionally does not blur the live app, avoiding whole-screen repaints during the transition. The sheet remains mounted and modal until its exit completes, avoiding the previous abrupt disappearance. Reduced-motion preferences collapse the animations to an effectively instant transition.
...
- Wheels are 44.2 px tall (37.4 px compact), 15% shorter than the original controls, with thin numbers, visible neighbours on a perspective cylinder, rotating grip ridges, and a fixed square drum-key selection marker. Disabled wheels use the shared Settings gray surface with neutral grip highlights, subtle cylindrical edge shading, and a gray read-only value. Activation updates the resting appearance once; it does not launch per-ridge, number, colour, or marker-filter fades. One animation loop controls rotation, inertia, and settling; cylinder transforms have no competing CSS transition. Angular shading belongs to individual ridges, never their preserve-3d parent, to avoid perspective shifts. Use `--trainer-accent` for selection/number colour. Inset 22 px minus/plus buttons and a center tap-to-type field share the existing wheel width; they add no grid columns or external spacing. The wheel surface accepts horizontal drags while preserving vertical page scrolling and pinch zoom. Tempo increase/interval wheels span 88–112 px; three-digit BPM typography scales to avoid touching neighbouring values. Reduced motion skips inertia and settling animations.
- Subdivisions use the approved notation across the rhythm sheet, rhythm pill, trainer preview, and stages. One notation group spans a written half, quarter, eighth, or sixteenth note, according to the meter denominator, with 1–13 clicks. Compact 76 × 44 px dropdowns use the Blue Olive accent and accessible musical-name/count labels. All meters also offer group-pulse-only playback. Bar counts remain numeric wheels. The meter selector ends with Custom…, revealing a compact inline editor for beats (1–16), note value (/2, /4, /8, /16), and grouping, with validation and Apply/Cancel. BPM is fixed to quarter notes. Quick uneven-group choices appear for 5/8, 7/8, 5/16, and 7/16. Static SVG noteheads derive from licensed Bravura outlines, without a full runtime font or extra animation.
...
- Wheels follow horizontal dragging directly at 24 px per step, with smooth momentum on release and a gentle final settle to the nearest number. Grabbing stops a rolling cylinder without rounding its position first. Numbers publish as they cross the center; parent echoes preserve ongoing movement. No detents, direction locks, special flick thresholds, or range-specific sensitivity apply. Native trackpad momentum remains intact. Blur/capture loss keeps the current selection.
...
- Dense accent patterns use four-meter-group paging and independent sheet scrolling.
