...
import { GAP_GLYPHS } from '../../assets/notation/gapGlyphs.js'
...
export default function SubdivisionNotation({ count, denominator = 4 }) {
  ...
  const { beams, hollow, tuplet, width } = getSubdivisionNotation(count, denominator)
  ...
  return <svg className="pulse-subdivision-notation" data-subdivision={count} width={width} height="54" viewBox={`0 0 ${width} 54`} aria-hidden="true" focusable="false">
    ...
    {Array.from({ length: count }, (_, index) => <g key={index}>
      <path d={hollow ? GAP_GLYPHS.noteheadHalf : NOTEHEAD} transform={`translate(${start + index * 14} 44) scale(.032 -.032)`} fill="currentColor" />
      <path d={`M${firstStem + index * 14} 43 V20`} fill="none" stroke="currentColor" strokeWidth="1.2" />
    </g>)}
    {count === 1 && beams > 0 ? <path data-flags={beams} d={GAP_GLYPHS[`flag${denominator}`]} transform={`translate(${firstStem - .6} 20) scale(.025 -.025)`} fill="currentColor" /> : Array.from({ length: beams }, (_, index) => <path key={index} d={`M${firstStem - .6} ${20 + index * (beams > 4 ? 4 : 5)} H${lastStem + .6}`} stroke="currentColor" strokeWidth="3" />)}
  </svg>
}
